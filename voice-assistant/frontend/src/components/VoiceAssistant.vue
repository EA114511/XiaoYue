...
const { isRecording, isProcessing, isPlaying, isStreamingResponse, status, transcript, response, mediaStream, startRecording, stopRecording, sendText, disconnect } = useVoiceChat({
  serverUrl,
  onTranscript(text) {
...
function addMessage(role, content, agent = null) {
  ...
  // 流式 LLM 响应期间复用同一条 assistant 消息，避免逐字产生多个气泡
  const last = messages.value[messages.value.length - 1]
  if (
    role === 'assistant' &&
...
// VAD 自动断句后，录音状态会先于用户松开按钮变为 false，
// 重置 pressing 标记避免状态不一致。
watch(isRecording, recording => {
  if (!recording && isPressing) {
    isPressing = false
  }
})
